...
/// Constructor
/// </summary>
CSkeletonBasics::CSkeletonBasics() : m_pD2DFactory(NULL),
...
int CSkeletonBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    wc.lpszClassName = "SkeletonBasicsAppDlgWndClass";
    ...
    while (WM_QUIT != msg.message)
    {
        ...
        while (PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE))
        {
            ...
            // Debugging the messages being processed
...
void CSkeletonBasics::Update()
{
    ...
    // std::cout << "CONNECTED Finish" << std::endl;
}
...
HRESULT CSkeletonBasics::CreateFirstConnected()
{
    ...
    int iSensorCount = 0;
    std::cout << "0: " << iSensorCount << std::endl;
    HRESULT hr = NuiGetSensorCount(&iSensorCount);
    std::cout << iSensorCount << std::endl;
    if (FAILED(hr))
    {
        std::cout << "Failed to detect any sensors in the first place" << std::endl;
        return hr;
    }

    std::cout << "We have detected sensors. Amount: " << iSensorCount << std::endl;
    ...
    for (int i = 0; i < iSensorCount; ++i)
    {
        ...
        if (FAILED(hr))
        {
            std::cout << "FAILED CONNECTION ATTEMPT: " << i << std::endl;
            continue;
        }

        std::cout << "Sensor successfully created" << std::endl;

        // Get the status of the sensor, and if connected, then we can initialize it
        hr = pNuiSensor->NuiStatus();
        if (S_OK == hr)
        {
            std::cout << "Sensor successfully connected and initialized" << std::endl;
            m_pNuiSensor = pNuiSensor;
            break;
        }
        else
        {
            std::cout << "Sensor failed connection, no initalization" << std::endl;
        }
...
{
    // std::cout << "Skeleton Processing Section" << NUI_SKELETON_COUNT << std::endl;
    ...
    {
        if (skeletonFrame.SkeletonData[i].eTrackingState != 0)
        {
            std::cout << "SkeletonNumber:" << i << std::endl;

            // Example: Print specific fields of the skeleton data
            std::cout << "SkeletonTrackingState:" << skeletonFrame.SkeletonData[i].eTrackingState << std::endl;
            for (int j = 0; j < 20; ++j)
            {
                std::cout << "SkeletonPosition:("
                          ...
                          << skeletonFrame.SkeletonData[i].SkeletonPositions[j].z << ")" << std::endl;
            }
        }
    }
}
...
HRESULT CSkeletonBasics::EnsureDirect2DResources()
{
    ...
    if (NULL == m_pRenderTarget)
    {
        ...
        if (width <= 0 || height <= 0)
        {
            SetStatusMessage(L"Invalid window dimensions for render target.");
